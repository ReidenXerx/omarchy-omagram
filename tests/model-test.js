...
test("a chat's info: subtitle, details, actions and tabs", () => {
  ...
  eq(M.infoActions(person, 1).map(a => a.id), ["mute", "search", "secret", "hearSound", "otherSound", "clear", "delete"])
...
test("readable colours: contrast, text that reads, ink on a fill", () => {
  ...
  eq([round(M.colorContrast(hex("#000000"), hex("#ffffff"))), round(M.colorContrast(hex("#777777"), hex("#777777")))], [21, 1])
  // Biscuit de Mar Dark, the theme this was found on: menu text and the accent both read poorly on the menu ground.
  const ground = hex("#2D2424"), menuText = hex("#9C8181"), text = hex("#DCC9BC"), accent = hex("#AE3F82")
  eq([M.colorContrast(menuText, ground) < 4.5, M.colorContrast(accent, ground) < 3], [true, true])
  eq(M.bestTextColor(ground, [menuText, text]), text, "the theme's own text colour reads better than its menu text")
  eq(M.colorContrast(M.readableColor(accent, ground, text, 4.5), ground) >= 4.5, true)
  eq(M.readableColor(text, ground, text, 4.5), text, "a colour that reads already stays as it is")
  eq(M.colorContrast(M.readableColor(hex("#333333"), ground, hex("#3a3a3a"), 4.5), ground) >= 4.5, true, "past the text colour when that is dim too")
  eq(M.colorContrast(M.inkOnFill(accent, [text, ground], 4.5), accent) >= 4.5, true)
  eq(M.inkOnFill(hex("#f5a524"), [text, hex("#1A1515")], 4.5), hex("#1A1515"), "a theme colour when one reads")
})

test("proxies: addresses, links, the steps of adding one, and its line", () => {
  eq(["proxy.example.com:1080", "1.2.3.4:443", "[2001:db8::1]:443", "host", "host:0", "host:70000", "bad host:80", ":80"].map(M.parseProxyAddress),
     [{ server: "proxy.example.com", port: 1080 }, { server: "1.2.3.4", port: 443 }, { server: "2001:db8::1", port: 443 }, null, null, null, null, null])
  eq(["https://t.me/proxy?server=a&port=1&secret=b", "t.me/socks?server=a&port=1", "tg://proxy?server=a", "https://t.me/durov", "proxy?server=a"]
       .map(M.isProxyLink), [true, true, true, false, false])
  eq([M.proxyLinkUrl("t.me/socks?server=a"), M.proxyLinkUrl("tg://proxy?server=a")], ["https://t.me/socks?server=a", "tg://proxy?server=a"])
  eq([M.proxySteps("mtproto").map(s => s.key), M.proxySteps("socks5").map(s => s.key), M.proxySteps("link").map(s => s.key)],
     [["address", "secret"], ["address", "username", "password"], ["link"]])
  const [address, secret] = M.proxySteps("mtproto")
  eq([M.proxyStepProblem(address, "host:80"), M.proxyStepProblem(address, "host") !== "", M.proxyStepProblem(secret, "dd0123456789abcdef0123456789abcdef"),
      M.proxyStepProblem(secret, "short") !== ""], ["", true, "", true])
  const socks = { id: 1, server: "a", port: 1080, type: "socks5", enabled: true }
  eq([M.proxyText(socks, { seconds: 0.123 }, "ready"), M.proxyText(Object.assign({}, socks, { enabled: false, type: "mtproto" }), { error: true }, "ready"),
      M.proxyText(socks, undefined, "proxy")], ["SOCKS5 · 123 ms · in use", "MTProto · not answering", "SOCKS5 · connecting…"])
  eq([M.connectionText("proxy"), M.connectionText("ready"), M.connectionText("")], ["Connecting to the proxy…", "Connected", ""])
})

test("a link's preview while typing, and disappearing messages", () => {
  eq(["see https://example.com/a?b=1 now", "t.me/durov", "go to example.com", "version 1.5.2", "plain words", "[site](example.org/x)"].map(M.composerLink),
     ["https://example.com/a?b=1", "t.me/durov", "example.com", "", "", "example.org/x"])
  eq([0, 5, 60, 3600, 86400, 172800, 604800, 1209600, 2678400, 31536000].map(M.autoDeleteText),
     ["Off", "5 seconds", "1 minute", "1 hour", "1 day", "2 days", "1 week", "2 weeks", "1 month", "1 year"])
  eq([0, 86400, 604800, 2678400, 172800].map(M.nextAutoDelete), [86400, 604800, 2678400, 0, 604800])
  eq(M.autoDeleteMenu({ id: 5, kind: "private", autoDelete: 604800 }).map(i => i.label),
     ["Keep messages", "After 1 day", "After 1 week   ✓", "After 1 month"])
  eq(M.autoDeleteMenu({ id: 5, kind: "private", autoDelete: 172800 }).map(i => i.label).slice(3), ["After 1 month", "After 2 days   ✓"])
  eq(M.autoDeleteMenu({ id: -5, kind: "secret", autoDelete: 0 }).map(i => i.label).slice(0, 2), ["Keep messages   ✓", "After 5 seconds once seen"])
  eq(["autoDelete:0", "autoDelete:604800", "autoDelete:-1", "mute:3600"].map(M.autoDeleteSeconds), [0, 604800, -1, -1])
  eq(M.infoActions({ id: 5, kind: "private", userId: 5, canSetAutoDelete: true }, 1).map(a => a.id).slice(0, 4), ["mute", "search", "autoDelete", "secret"])
})
...
test("one reaction over a selection is a single toggle, not one per message", () => {
  const liked = { reactions: [{ emoji: "\u2764", chosen: true }] }
  const list = [msg(1, liked), msg(2, liked), msg(3)]
  eq(M.selectedMessages(list, { 1: true, 3: true }).map(m => m.id), [1, 3])
  eq(M.selectedMessages(list, {}), [])
  assert.ok(M.reactionAdds(M.selectedMessages(list, { 1: true, 3: true }), "\u2764"),
            "one of them is not reacted to yet, so the whole selection gets it")
  assert.ok(!M.reactionAdds(M.selectedMessages(list, { 1: true, 2: true }), "\u2764"),
            "every one already carries yours, so it comes off")
  assert.ok(M.reactionAdds([], "\u2764"), "nothing selected is not 'already reacted'")
})
...
test("a round video's moving picture keeps up with its sound", () => {
  assert.strictEqual(M.noteFrame(0, 1, 20, 80), 0)
  assert.strictEqual(M.noteFrame(1000, 1, 20, 80), 20)
  assert.strictEqual(M.noteFrame(1000, 1.5, 20, 80), 30)
  assert.strictEqual(M.noteFrame(60000, 2, 20, 80), 79, "never past the last frame")
  assert.strictEqual(M.noteFrame(-500, 1, 20, 80), 0, "a start still to come is the first frame")
  assert.strictEqual(M.noteFrame(1000, 0, 20, 80), 20, "no rate is normal speed")
  assert.strictEqual(M.noteFrame(1000, 1, 0, 80), 0)
  assert.strictEqual(M.noteFrame(1000, 1, 20, 0), 0)
  assert.strictEqual(M.noteFrame("soon", NaN, 20, 80), 0)
})
